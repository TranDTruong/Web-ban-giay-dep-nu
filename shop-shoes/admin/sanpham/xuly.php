	...
	// Thêm Sản Phẩm
		if(isset($_POST['xlthem'])){
			...
			if(isset($_POST['mota'])){$mota=$_POST['mota'];}else{$mota=NULL;}
     		$sql_them=" INSERT INTO `sanpham`(`MaSP`, `TenSP`, `MaDM`, `MaNCC`, `MoTa`, `DonGia`,`SoLuong`, `AnhNen`) 
						VALUES (NULL,'$tensp','$madm', '$mancc', '$mota', '$dongia','$soluong','$anhnen')";
			//  $sql_them=" INSERT INTO `sanpham`(`TenSP`, `MaDM`, `MaNCC`, `MoTa`, `DonGia`,`SoLuong`, `AnhNen`) 
			//  			VALUES ('".$tensp."','".$madm."', '".$mancc."', '".$mota."', '".$dongia."','".$soluong."','".$anhnen."')";
						//  VALUES ('".$data['name']."','".$data['description']."','".$data['thumbnail']."','".$data['slug']."','".$data['creat_at']."')";
			// var_dump($sql_them);
			// die();
